...
namespace Packet_WorldDrawing
{
    ...
    public class MyDrawings
    {
        ...
        {
            mypencils[nPencil].setPencil(finish, thick, outter);
            myshapes[nShape] = mypencils[nPencil];
        }
        public void SetMyLine()
        {
            if(!Colored)
                mylines[nline].setPoint(start, finish,thick,outter);
            else
                mylines[nline].setPoint(start, finish, thick, outter,inner);
        ...
        {
            if(!Colored)
                mycircle[ncircle].setRectC(start, finish, thick, outter);
            else
                mycircle[ncircle].setRectC(start, finish, thick, inner, outter);
